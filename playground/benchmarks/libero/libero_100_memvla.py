...
class LiberoMemVLATrainerConfig(MemVLATrainerConfig):
    output_dir: str = field(
        default=f'./user_checkpoints/dexbotic/libero_100_memvla/all-{datetime.now().strftime("%m%d")}')
    wandb_project: str = field(default='dexbotic_libero_100_memvla')
...
class LiberoMemVLADataConfig(MemVLADataConfig):
    dataset_name: str = field(default='libero_10+libero_90')
...
class LiberoMemVLAInferenceConfig(InferenceConfig):
    ...
    model_name_or_path: str = field(
        default='./checkpoints/libero/libero_100_memvla')
