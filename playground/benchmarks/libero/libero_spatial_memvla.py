...
class LiberoMemVLATrainerConfig(MemVLATrainerConfig):
    output_dir: str = field(
        default=f'./user_checkpoints/dexbotic/libero_spatial_memvla/all-{datetime.now().strftime("%m%d")}')
    wandb_project: str = field(default='dexbotic_libero_spatial_memvla')
    num_train_epochs: int = field(default=75)
...
class LiberoMemVLADataConfig(MemVLADataConfig):
    dataset_name: str = field(default='libero_spatial')
...
class LiberoMemVLAInferenceConfig(InferenceConfig):
    ...
    model_name_or_path: str = field(
        default='./checkpoints/libero/libero_spatial_memvla')
